...
const CanvasWidget: React.FC<CanvasWidgetProps> = ({
  ...
  void availableCols;
    ...
    if (widget.type === 'supabase') {
      ...
      return (
        <div
          className="flex flex-col items-center justify-center w-full h-full cursor-default py-2"
          onClick={(e) => e.stopPropagation()}
        >
          {/* Single circle with icon - fills the outer widget circle */}
          <div className="flex flex-col items-center justify-center gap-2">
            {/* Icon circle matching the outer widget container size (80x80) */}
            <div 
            ...
            <div className="text-center">
              {widget.data?.fetchStatus === 'fetching' ? (
                <div className="text-xs text-blue-600 font-medium">Fetching...</div>
              ) : hasData ? (
                <div className="text-xs text-green-600 font-medium">
                  {tableData.length} instances
                </div>
              ) : (
          ...
          <div className="w-full px-3 mt-3 flex flex-col gap-2">
            {!hasData && (
              <>
                <input 
                  className="w-full rounded px-3 py-2 text-sm border border-gray-300 focus:border-orange-500 focus:outline-none text-center bg-white text-gray-800 font-medium placeholder-gray-400" 
                  value={sbTableName} 
                  onChange={(e) => setSbTableName(e.target.value)} 
                  placeholder="raman_data" 
                />
                <input 
                  className="w-full rounded px-3 py-2 text-sm border border-gray-300 focus:border-blue-500 focus:outline-none text-center bg-white text-gray-800 placeholder-gray-400" 
                  value={sbSampleFilter} 
                  onChange={(e) => setSbSampleFilter(e.target.value)} 
                  placeholder="Sample name (optional)" 
                />
                <button 
                  className="w-full px-3 py-2 rounded bg-orange-500 text-white text-sm font-semibold hover:bg-orange-600 disabled:bg-gray-300 disabled:cursor-not-allowed transition-colors shadow-sm" 
            ...
            {hasData && (
              <button 
                className="w-full px-3 py-2 rounded bg-green-500 text-white text-sm font-semibold hover:bg-green-600 transition-colors shadow-sm" 
                onClick={(e) => { 
                  e.stopPropagation(); 
